...
async function main() {
  ...
  const allCls = [];
  h.classes.forEach((val, key) => allCls.push(key));
  console.log('Hierarchy classes count:', allCls.length);
  const statusClasses = allCls.filter(c => c.toLowerCase().includes('status') || c.toLowerCase().includes('state'));
  console.log('Status/State classes:', statusClasses);

  for (const sc of statusClasses) {
    try {
      const items = await tx.findAll(sc, {});
      console.log(sc, '(' + items.length + '):', items.map(i => ({ id: i._id, name: i.name, title: i.title, key: i.key })));
    } catch(e) {}
  }
